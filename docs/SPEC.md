...
export const ASSISTANT_NAME = process.env.ASSISTANT_NAME || 'Luzia365';
...
Messages must start with the trigger pattern (default: `@Luzia365`):
- `@Luzia365 what's the weather?` → ✅ Triggers Claude
- `@luzia365_bot help me` → ✅ Triggers (case insensitive)
- `Hey @Luzia365` → ❌ Ignored (trigger not at start)
...
[Jan 31 2:35 PM] John: @Luzia365 what toppings do you recommend?
```
...
|---------|---------|--------|
| `@Assistant [message]` | `@Luzia365 what's the weather?` | Talk to Claude |

### Commands Available in Main Channel Only

| Command | Example | Effect |
|---------|---------|--------|
| `@Assistant add group "Name"` | `@Luzia365 add group "Family Chat"` | Register a new group |
| `@Assistant remove group "Name"` | `@Luzia365 remove group "Work Team"` | Unregister a group |
| `@Assistant list groups` | `@Luzia365 list groups` | Show registered groups |
| `@Assistant remember [fact]` | `@Luzia365 remember I prefer dark mode` | Add to global memory |
...
```
User: @Luzia365 remind me every Monday at 9am to review the weekly metrics
...
```
User: @Luzia365 at 5pm today, send me a summary of today's emails
...
From any group:
- `@Luzia365 list my scheduled tasks` - View tasks for this group
- `@Luzia365 pause task [id]` - Pause a task
- `@Luzia365 resume task [id]` - Resume a paused task
- `@Luzia365 cancel task [id]` - Delete a task

From main channel:
- `@Luzia365 list all tasks` - View tasks from all groups
- `@Luzia365 schedule task for "Family Chat": [prompt]` - Schedule for another group
...
| "No groups registered" | Haven't added groups | Use `@Luzia365 add group "Name"` in main |
